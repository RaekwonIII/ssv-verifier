...
export interface VerifyConfigDependencies {
...
export async function verifyNetworkConfig(
  ...
  dependencies: VerifyConfigDependencies = {},
): Promise<VerifyConfigResult> {
  if (config.activeNetworks.length !== 1) {
    throw new Error("verify-config requires a single network target, not --network both.");
  }
...
export function renderVerifyConfigSummary(result: VerifyConfigResult): string {
  const lines = [
    `verify-config ${result.status.toUpperCase()}`,
